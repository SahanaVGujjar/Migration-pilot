...
exports.JsToTsTransformer = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class JsToTsTransformer {
    constructor(ai) {
    ...
    convertRequiresToImports(content) {
        const changes = [];
        let changed = false;
        let linesAdded = 0;
        let linesRemoved = 0;
        ...
        result = result.replace(/const\s+(\w+)\s*=\s*require\s*\(\s*['"]([^'"]+)['"]\s*\)\s*;?/g, (_match, varName, modulePath) => {
            changed = true;
            changes.push(`Converted require('${modulePath}') to import`);
            return `import ${varName} from '${modulePath}';`;
        });
        // const { a, b } = require('module')
        result = result.replace(/const\s+(\{[^}]+\})\s*=\s*require\s*\(\s*['"]([^'"]+)['"]\s*\)\s*;?/g, (_match, destructured, modulePath) => {
            changed = true;
        ...
        result = result.replace(/const\s+(\w+)\s*=\s*require\s*\(\s*['"]([^'"]+)['"]\s*\)\.(\w+)\s*;?/g, (_match, varName, modulePath, prop) => {
            changed = true;
    ...
    convertExports(content) {
        ...
        result = result.replace(/module\.exports\s*=\s*(\w+)\s*;?/g, (_match, name) => {
            changed = true;
            changes.push(`Converted module.exports to export default`);
            return `export default ${name};`;
        });
        // module.exports = { a, b } → export { a, b }
        result = result.replace(/module\.exports\s*=\s*(\{[^}]+\})\s*;?/g, (_match, obj) => {
            changed = true;
    ...
    async addFunctionTypes(content, filePath) {
        const changes = [];
        let changed = false;
        let linesAdded = 0;
        let result = content;
        // Add return type void to functions that don't return anything
        result = result.replace(/function\s+(\w+)\s*\(([^)]*)\)\s*\{/g, (match, funcName, params) => {
        ...
        result = result.replace(/(const|let|var)\s+(\w+)\s*=\s*\(([^)]*)\)\s*=>/g, (match, keyword, funcName, params) => {
            ...
            try {
                ...
                for (const typeInfo of aiTypes) {
                    if (typeInfo.confidence > 0.7 && typeInfo.inferredType !== 'any') {
                        const pattern = new RegExp(`(${typeInfo.variableName})(?!\\s*:)\\s*([,)=])`, 'g');
                        const newResult = result.replace(pattern, `$1: ${typeInfo.inferredType}$2`);
                        if (newResult !== result) {
                            result = newResult;
                            changed = true;
                            linesAdded++;
                            changes.push(`AI inferred ${typeInfo.variableName}: ${typeInfo.inferredType} (${Math.round(typeInfo.confidence * 100)}% confidence)`);
                        }
                    }
                }
            }
    ...
    inferParamTypes(params, fullContent) {
        if (!params.trim())
            return params;
        return params
            .split(',')
    ...
    inferTypeFromDefault(defaultVal) {
    ...
    inferTypeFromUsage(paramName, content) {
        ...
        if (mapCheck.test(content))
            return 'any[]';
        if (stringMethods.test(content))
            return 'string';
        if (numberOps.test(content))
            return 'number';
        if (booleanCheck.test(content))
            return 'boolean';
        if (lengthCheck.test(content))
            return 'string | any[]';
        return 'any';
    }
    functionHasReturn(content, funcMatch) {
    ...
    addVariableTypes(content) {
        ...
        result = result.replace(/(const|let)\s+(\w+)\s*=\s*(true|false)\s*;/g, (_match, keyword, name, val) => {
            changed = true;
            return `${keyword} ${name}: boolean = ${val};`;
        });
        result = result.replace(/(const|let)\s+(\w+)\s*=\s*(\d+(?:\.\d+)?)\s*;/g, (_match, keyword, name, val) => {
            changed = true;
            return `${keyword} ${name}: number = ${val};`;
        });
        result = result.replace(/(const|let)\s+(\w+)\s*=\s*(['"`][^'"`]*['"`])\s*;/g, (_match, keyword, name, val) => {
            changed = true;
            return `${keyword} ${name}: string = ${val};`;
        });
        result = result.replace(/(const|let)\s+(\w+)\s*=\s*new\s+(Map|Set|WeakMap|WeakSet|Array)\s*\(\s*\)\s*;/g, (_match, keyword, name, cls) => {
            changed = true;
            const typeMap = {
                'Map': 'Map<any, any>',
                'Set': 'Set<any>',
    ...
    fixImportExtensions(content) {
        ...
        const result = content.replace(/(from\s+['"])([^'"]+)\.(js|jsx)(['"])/g, (_match, prefix, modPath, _ext, suffix) => {
            changed = true;
    ...
    convertPropTypes(content) {
        ...
        for (const match of propTypesMatches) {
            ...
            if (props.length > 0) {
                ...
                result = result.replace(/import\s+PropTypes\s+from\s+['"]prop-types['"];\s*\n?/, '');
                changed = true;
                changes.push(`Converted PropTypes to TypeScript interface ${interfaceName}`);
            }
        }
        return { content: result, changed, changes };
    }
    ...
    isReactFile(content) {
        return (/import\s+.*React/.test(content) ||
            /from\s+['"]react['"]/.test(content) ||
            /<\w+[\s/>]/.test(content) ||
            /jsx|tsx/.test(content));
    }
}
exports.JsToTsTransformer = JsToTsTransformer;
